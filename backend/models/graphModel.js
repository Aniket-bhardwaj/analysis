...
class graphModel {
  ...
  static queryGraphRowsByFileId(fileId, element, isAdmin, orgId) {
    return new Promise((resolve, reject) => {
      db.get(
        ...
        (err, fileRow) => {
          ...
          // RBAC check for non-admins
          if (!isAdmin && fileRow.org_id !== orgId) {
            return reject(new Error('Unauthorized access to this file'));
          }

          const fileType = fileRow.type;
          const qcLabel = VALID_LABELS[fileType];
          if (!qcLabel) return reject(new Error('Invalid file type'));
  ...
  static queryGraphDataByDateRange(startDate, endDate, element, isAdmin, orgId, solutionLabel = null) {
    return new Promise((resolve, reject) => {
      ...
      const query = `
        ...
        WHERE f.uploaded_at BETWEEN ? AND ?
          AND f.hidden = 0
          AND ((? = 1) OR (? = 0 AND f.org_id = ?))
          AND q."Solution Label" ${solutionLabel ? "= ?" : "LIKE 'QC%'"}
        ORDER BY ${timeColumn} ASC
      `;

      const params = solutionLabel
        ? [start, end, isAdmin ? 1 : 0, isAdmin ? 1 : 0, orgId, solutionLabel]
        : [start, end, isAdmin ? 1 : 0, isAdmin ? 1 : 0, orgId];
  ...
  static fetchSJSGraphDataByDateRange(startDate, endDate, isAdmin, orgId) {
    return new Promise((resolve, reject) => {
      const start = `${startDate} 00:00:00`;
      const end = `${endDate} 23:59:59`;
      ...
      db.all('SELECT * FROM sjs', (err, sjsRows) => {
        ...
        const params = [start, end, isAdmin ? 1 : 0, isAdmin ? 1 : 0, orgId];
        ...
        db.all(query, params, (err2, rows) => {
          ...
          rows.forEach(row => {
            ...
            sjsElements.forEach(el => {
              ...
              const point = { x: time, y: val, mid, upper: mid + error, lower: mid - error };
