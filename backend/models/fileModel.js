  ...
  return new Promise((resolve, reject) => {
    ...
    db.run(
      ...
      [
        ...
        season, // 🆕 include season value in insert
      ],
      function (err) {
...
function getVisibleFiles() {
  const sql = `
    SELECT
      f.id, f.filename, f.file_path, f.pdfname, f.pdf_path, f.uploaded_at,
      f.type, f.hidden, f.org_id, f.created_by_user_id, f.season,  -- 🆕 added
...
function getFileTypesByDateRange(startDate, endDate, isAdmin = false, orgId = null) {
  ...
  const params = isAdmin
    ...
    : [`${startDate} 00:00:00`, `${endDate} 23:59:59`, orgId];
  return new Promise((resolve, reject) => {
...
function getFileByNameForUser(filename, isAdmin, orgId) {
  const sql = `
    SELECT
      f.id, f.filename, f.file_path, f.pdfname, f.pdf_path, f.uploaded_at,
      f.type, f.hidden, f.org_id, f.created_by_user_id, f.season,  
      u.email  AS owner_email,
      o.name   AS org_name
    FROM uploaded_files f
    LEFT JOIN users u ON f.created_by_user_id = u.id
...
function getFileMetadata(fileId) {
  const sql = `
    SELECT filename, uploaded_at, type, season  -- 🆕 added
    FROM uploaded_files
    WHERE id = ?
  `;
  return new Promise((resolve, reject) => {
