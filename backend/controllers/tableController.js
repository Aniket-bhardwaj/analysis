...
class TableController {
  ...
  static async getTableDataByFile(req, res) {
    try {
      ...
      console.log(`[TableController] Fetching QC table data for file_id: ${file_id}, solution_label: ${solutionLabel}`);
      ...
      if (!result.tableData || result.tableData.length === 0) {
        return res.json({
          success: true,
          message: result.message || 'No data found for the specified criteria',
          tableData: [],
          elements: [],
  ...
  static async getSolutionLabelsForTable(req, res) {
    try {
      ...
      res.json({
        ...
        summary: {
          totalQCLabels: solutionLabels.qcLabels ? solutionLabels.qcLabels.length : 0,
          totalSampleLabels: solutionLabels.sampleLabels ? solutionLabels.sampleLabels.length : 0,
          hasQCData: solutionLabels.qc && solutionLabels.qc.length > 0,
          hasSampleData: solutionLabels.sample && solutionLabels.sample.length > 0
        }
      });
