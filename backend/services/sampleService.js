...
const typeCorr = { 1: MEcorr, 2: TEcorr };
const typeO = { 1: MEconc, 2: TEconc };
...
class SampleService {
  // 🔹 Element details with RBAC
  static async getSampleElementDetails(sampleId, elementName, { isAdmin, orgId }) {
    try {
      const cleanedElementName = elementName.replace(/_Corrected$/, "");
      const csvType = cleanedElementName.includes("ppb") ? 2 : 1;

      // RBAC-aware file lookup
      const fileId = await SampleModel.getFileIdForSampleAndType(
        sampleId,
        csvType,
        isAdmin,
        orgId
      ...
      if (!fileId) throw new Error("No matching file found for this type.");

      // RBAC-aware avg/rsd fetch
      ...
      return { element: cleanedElementName, type: csvType, file_id: fileId, avgRow, rsdRow };
    } catch (error) {
      console.error("Error in getSampleElementDetails:", error);
      throw error;
    }
  }

  // 🔹 Full sample table with RBAC
  static async getSampleTableData(sampleId, { isAdmin, orgId }) {
    try {
      console.log("Fetching sample table for sampleId:", sampleId);

      // RBAC-aware file lookup
      const files = await SampleModel.getFileTypesWithIdBySampleId(
        sampleId,
        isAdmin,
        orgId
      );
      ...
      const fileLinks = await SampleModel.getFileNamesByFileIds(
        fileIds,
        isAdmin,
        orgId
      );
      ...
      if (files.length === 1) {
        ...
        const avgrow = await SampleModel.getAvg(file_id, qcl[type], typeO[type], isAdmin, orgId);
        const errorobj = await generateErrorCheckRows(qcl[type], avgrow);
        const merged = await mergeCorrectedValues(errorobj, row);

        return { tableData: merged, fileLinks };
      } else {
        const file1 = files.find((f) => f.type === 1);
        const file2 = files.find((f) => f.type === 2);

        if (!file1 || !file2) throw new Error("Required file types 1 and 2 not found for sample");
        ...
        const avgrow1 = await SampleModel.getAvg(file1.file_id, qcl[1], typeO[1], isAdmin, orgId);
        const avgrow2 = await SampleModel.getAvg(file2.file_id, qcl[2], typeO[2], isAdmin, orgId);
        ...
        return { tableData: [...merged1, ...merged2], fileLinks };
      }
    } catch (error) {
      console.error("Error in getSampleTableData:", error);
      throw error;
    }
  }
}
