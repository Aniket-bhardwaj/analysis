...
class DownloadService {
  ...
  async createZipWithCSVs(fileId, rbac = {}) {
    ...
    const originalPath = resolveFilePath(fileInfo.file_path);
    if (!originalPath) {
      throw new Error(
    ...
    try {
      const attachments = await new Promise((resolve, reject) => {
        db.all(
          `SELECT id, filename, file_path FROM uploaded_files 
      ...
      if (attachments.length > 0) {
        console.log(`[ZIP] Adding ${attachments.length} attachments for file ${fileId}`);
        for (const att of attachments) {
          if (!att.file_path) continue;
          const abs = resolveFilePath(att.file_path);
          if (abs && fs.existsSync(abs)) {
            zip.addLocalFile(abs, "attachments", path.basename(abs));
          } else {
            console.warn(`[ZIP] Skipped missing attachment ${att.file_path}`);
          }
        }
      }
    } catch (err) {
      console.error(`[ZIP] Failed to include attachments:`, err);
    }
