...
export default function ManageAccessPage() {
  ...
  const [form, setForm] = useState({ name: "", email: "", org_id: "", isAdmin: false });
  ...
  const handleAdd = async () => {
    try {
      if (tab === 0) {
        // Add user
        const orgToUse = form.org_id || selectedOrg;
        if (!form.email || !orgToUse) {
        ...
        const res = await jsonPost(
          ...
          { email: form.email.trim(), orgId: orgToUse, isAdmin: form.isAdmin ? true : false }
        );
        ...
        if (res.ok) {
          ...
          fetchUsers(orgToUse);
          setOpenAdd(false);
          setForm({ name: "", email: "", org_id: "", isAdmin: false });
        } else {
  ...
  return (
    <Box className="dashboard-container file-upload-container">
        ...
        <Paper sx={{ p: 3, borderRadius: 3, boxShadow: 2 }}>
          ...
          <Box sx={{ mt: 2 }}>
            <Button
              ...
              startIcon={<Add />}
              onClick={() => {
                setForm({ name: "", email: "", org_id: selectedOrg || "", isAdmin: false });
                setOpenAdd(true);
              }}
              sx={{ mb: 2 }}
            >
      ...
      <Dialog open={openAdd} onClose={() => setOpenAdd(false)}>
        <DialogTitle>
          Add {tab === 0 ? `User${selectedOrg ? ` to ${orgs.find(o => o.id == selectedOrg)?.name || ''}` : ''}` : "Organization"}
        </DialogTitle>
        ...
        <DialogContent>
          {tab === 0 ? (
            <>
              ...
              {/* Show admin checkbox only for Main Lab (ID = 1) */}
              {String(form.org_id || selectedOrg) === "1" && (
                <Box sx={{ mt: 1, display: "flex", alignItems: "center" }}>
                  ...
                  />
                  <label htmlFor="isAdmin">Grant Admin Privileges (Main Lab only)</label>
                </Box>
              )}
            </>
          ) : (
