...
const SJSTable = ({ selectedFileId, selectedDateRange }) => {
  ...
  const toggleRowExpansion = (element) => {
    ...
    if (next.has(element)) {
      ...
      next.add(element);
      if (!miniTables[element] || !miniTables[element].data?.length) {
  ...
  const sortMiniTable = (element, key) => {
    ...
    setMiniTables((prev) => ({
      ...prev,
      [element]: { ...prev[element], data: sortedData },
    }));
  ...
  // Whitelist of known keys for ordering
  ...
  const sortableKeys = preferredColumns.map((c) => c.key).filter(Boolean);
  const dynamicColumns = preferredColumns;

  // // If backend sends extra keys, add them after preferred ones
  // const dynamicColumns = useMemo(() => {
  //   if (!sjsData.length) return preferredColumns;
  //   const firstRow = sjsData[0];
  //   const extraKeys = Object.keys(firstRow).filter(
  //     (k) => !preferredColumns.find((c) => c.key === k)
  //   );
  //   return [
  //     ...preferredColumns.filter((c) => c.key !== null),
  //     ...extraKeys.map((k) => ({ key: k, label: k })),
  //     preferredColumns.find((c) => c.key === null), // Status always last
  //   ];
  // }, [sjsData]);
    ...
    <Box sx={{ px: 2, pt: 2 }}>
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'baseline', pl: 1 }}>
          <Typography variant="h6">Standard Quality Analysis</Typography>
...
//       if (
//         !miniTables[element] ||
//         !miniTables[element].data ||
//         miniTables[element].data.length === 0
//       ) {
//         fetchMiniTableData(element, 1);
//       }
//     }
...
//           {sortedData.map((row, index) => (
//             <SJSRow
//               key={`${row.fullElementName || row.element}-${index}`}   // 👈 Fix
//               row={row}
//               isExpanded={expandedRows.has(row.fullElementName)}
//               toggleRowExpansion={toggleRowExpansion}
//               miniTableData={miniTables[row.fullElementName] || {}}
