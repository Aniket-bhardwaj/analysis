...
const QCTable = ({ selectedFileId, selectedDateRange }) => {
  ...
  //   if (!selectedFileId) return;   // guard
  //   const res = await apiFetch(`/qc-check/summary?file_id=${selectedFileId}`);
  //   if (res.success) {
  //     console.log("QC API Response:", res); // 👀 log raw response

  //     const normalized = res.qcData.map(r => ({
  //       element: r.element,
  //       fullElementName: r.fullElementName,
  //       valueAvg: r.valueAvg ?? r.value_avg ?? null,
  //       correctedValueAvg: r.correctedValueAvg ?? r.corrected_value_avg ?? null,
  //       rsd: r.rsd ?? null,
  //       errorPercentage: r.errorPercentage ?? r.error_percentage ?? null,
  //       solutionLabel: r["Solution Label"] ?? null,
  //       status: r.isWithinTolerance ? "Pass" : "Fail"
  //     }));
